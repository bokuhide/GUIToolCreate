...
\Etitle{Realization of the GUI editor the orchestration definition is made easy for which by the OpenStack environment}
\idnumber{1160304}
\author{川口　貴大}
...
\abstract{
	...
	OpenStackは最も開発が進んでいるIaaS基盤ソフトウェアの一つである．OpenStackはHeatと呼ばれるコンポーネントでシステム自動構築をする．Heatは，ITリソースの構成情報を記述した設計図(テンプレートファイル)を読み込ませることで，その構成情報を基に自動的にシステムの構築・起動を行う．そのため，テンプレートファイルの作成はシステムを構築する上で重要な役割を担っている．しかし，テンプレートファイルは書式が複雑であり，記述を行う際にはHeat独自の知識を要する．またテキストファイルであるため，記述量が膨大になり，さらに構成要素の関係が把握し難い問題もある．
...
\Eabstract{
	Recentry，there has been a growing demand for IaaS．The reason is rapid securing of IT resources， cost reduction．A system usually consists of more than one machine, and as it becomes large-scale, it takes time for a definition.
	
	OpenStack is one of IaaS foundation software．That has been developed most．OpenStack builds a system automatically by the component called "Heat"．When the plan in which composition information on an IT resource was written (template file) is read, Heat does building of a system and a start automatically based on the composition information．Therefore when building a system， the making of a template file is charged with the important role．But a form is complicated and when describing， a template file needs original knowledge．There is a problem that the description amount often becomes enormous because it's a text file. When the description amount increases, and becomes enormous, there is also a problem that a relation of a component is difficult to grasp.
	
	Experimental results，shows GUI editor settles a conventional problem, than a usual method, a short time and, a system could be defined easily.
